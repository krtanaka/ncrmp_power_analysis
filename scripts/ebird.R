...
# ebird data
ebird <- read_csv("/Users/Kisei.Tanaka/Desktop/data/ebd_woothr_june_bcr27_zf.csv") %>% 
...
habitat <- read_csv("/Users/Kisei.Tanaka/Desktop/data/pland-elev_location-year.csv") %>% 
...
# prediction surface
pred_surface <- read_csv("/Users/Kisei.Tanaka/Desktop/data/pland-elev_prediction-surface.csv")
# latest year of landcover data
max_lc_year <- pred_surface$year[1]
r <- raster("/Users/Kisei.Tanaka/Desktop/data/prediction-surface.tif")

# load gis data for making maps
map_proj <- st_crs("ESRI:102003")

ne_land <- read_sf("/Users/Kisei.Tanaka/Desktop/data/gis-data.gpkg", "ne_land") %>% 
  st_transform(crs = map_proj) %>% 
  st_geometry()

bcr <- read_sf("/Users/Kisei.Tanaka/Desktop/data/gis-data.gpkg", "bcr") %>% 
  st_transform(crs = map_proj) %>% 
  st_geometry()

ne_country_lines <- read_sf("/Users/Kisei.Tanaka/Desktop/data/gis-data.gpkg", "ne_country_lines") %>% 
  st_transform(crs = map_proj) %>% 
  st_geometry()

ne_state_lines <- read_sf("/Users/Kisei.Tanaka/Desktop/data/gis-data.gpkg", "ne_state_lines") %>% 
  st_transform(crs = map_proj) %>% 
  st_geometry()
